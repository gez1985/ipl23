...
const ShortlistPlayerButton = ({ playerId }) => {
  ...
  const handleAddPlayerClick = async () => {
    if (!addPosition || addPosition < 1) {
      ...
      try {
        ...
        if (addPosition > manager.shortlist.length) {
          managerCopy.shortlist.push(playerId);
        } else if (addPosition === 1) {
          managerCopy.shortlist.unshift(playerId);
        } else {
          managerCopy.shortlist.splice(addPosition - 1, 0, playerId);
        }
...
export default ShortlistPlayerButton;
