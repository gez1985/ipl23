...
const TransportListPlayerButton = ({ playerId }) => {
  ...
  const handleAddPlayerClick = async () => {
    if (!addPosition || addPosition < 1) {
      ...
      try {
        ...
        if (addPosition > manager.transfers.length) {
          managerCopy.transfers.push(playerId);
        } else if (addPosition === 1) {
          managerCopy.transfers.unshift(playerId);
        } else {
          managerCopy.transfers.splice(addPosition - 1, 0, playerId);
        }
  ...
  return (
    <div className="ss-add-player-container" style={{ width: "150px" }}>
      ...
      />
      <button
        className="shortlist-add-button shortlist-keep    "
...
export default TransportListPlayerButton;
