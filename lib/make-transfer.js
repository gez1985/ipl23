...
const updateTransferHistory = async (managerId, playerOutId, playerInId) => {
  try {
    const sql =
      "INSERT INTO transfers (manager_id, player_out_id, player_in_id) VALUES ($1, $2, $3) RETURNING *";
    const values = [managerId, playerOutId, playerInId];
...
const removeTransferredPlayer = async (manager) => {
  ...
  if (index > -1) {
    manager.stage1Squad.splice(index, 1);
  }
};
